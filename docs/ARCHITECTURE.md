...
版本 0.1.0（build 1），对齐 Windows v1.27.0，使用原生 InputMethodKit / AppKit 前端和 Free Pascal 独立引擎。
...
异常拼音范围通过结果载荷 schema 2/3 传递，范围为 UTF-16；无警告且无补全来源时使用 schema 1。原生前端给对应标记文本添加红色文字与下划线颜色；IMK 或应用可能重建并替换这些属性，因此候选窗第二行同时用红色和下划线显示异常拼音片段。仅错误时显示，修正后清除，不改变输入文本、选区、两行高度、Tab 补全和品牌区。长输入截取首个错误附近的片段，完整拼音保留在提示文本中。正常音节、未完成声母和双拼编码按各自方案判断；重复元音只在有足够输入证据时提示。

核心对齐 Windows 1.27.0，保留 Linux 的 FPC 适配与 macOS 的组合串语义。预测前缀准入由 `nc_prefix_completion_policy` 管理，候选快照和导航稳定性由 `nc_candidate_presentation` 管理；鼻音边界、已确认后缀、短复合词上下文证据、ü 别名和冷启动分页均有独立回归。共享轻量排序的两个局部缓存标志在入口显式初始化。

模型参数和九个 ONNX 图与 Windows 1.27.0 的部署资产相同，ORT 1.20.1 固定 CPU 执行、量化精度配置和线程/内存策略。生产补全结果预算为 50 ms，局部修正为 30 ms；主排序器的 30 ms 仅记录审计，保留已经同步算出的结果，与 Linux 宿主一致。没有引入云端、GPU/CoreML 执行器或重新训练。
...
基础数据库只读，用户数据库使用独立可写路径。Lexicon 1.27.0 的 24 个输入通过 schema 24 导入器重建简繁两库，完整保留 query paths、转移、补全竞争/审计和正反字符语言模型；模型与 schema 的 SHA-256 在 `data/runtime-assets.sha256`，词库生成输入校验值在 `data/lexicon-inputs.json`。
...
首屏全拼没有预测时，`nc_tab_exact_fallback` 可把已输入的两个精确词段拼接为 Tab 结果。头尾都必须是完整词并逐字符合当前音节，明确分隔符不能被重切，重复完整候选不展示；翻页和编辑会失效缓存。确认该结果不新增用户词或补全学习反馈。预测整段使用主题强调色，精确拼接整段使用普通文字色。

结果 schema 3 保留 schema 2 的警告列表，追加补全来源 byte、零保留 byte 和零保留 uint16。来源 1–6 为预测，7 为精确拼接；非空补全才能携带来源。新前端读取 schema 1/2 时把旧非空补全视为预测；清空上下文同时清除来源。升级时前端和引擎一起替换。

缓存词链复用需要按当前音节逐段验证，五音节也进入精确词链搜索；新快速分支保留有转移证据的四字复合前缀。字符 LM 冷缓存仅补载最多 64 段、每段 40 个 UTF-16 单位；前缀学习使用精确键的有界重复选择证据，先复制缓存候选再排序。
