...
版本 0.1.0（build 1），对齐 Windows v1.26.1，使用原生 InputMethodKit / AppKit 前端和 Free Pascal 独立引擎。
...
新增联合局部纠错在已有词典路径保护后选择候选，并由双向模型复核。查询编码与原有局部修正共享，双向复核保留原始 FP32 编码；字符、词频和路径约束保持有效。文档上下文存在时保留原上下文修正路径。

长句补全可在最终修正阶段之前预取；工作结果只在请求全部字段、context 和 generation 相同时复用一次。显示和提交 Tab 时，可将已经验证且不改变尾部锚点的前缀修正投影到补全，不修改排序证据或未输入后缀。

紫光双拼分别转换声母与韵母；未完成全拼保留完整 ng 音节，并正确消费混合简拼的翘舌声母。共享用户词按当前简繁模式转换显示，同拼音的基础词库优先确定多义字形；学习仍更新原有记录，删除同时处理简繁别名。

异常拼音范围通过结果载荷 schema 2 传递，范围为 UTF-16；无警告仍使用 schema 1。原生前端给对应标记文本添加红色文字与下划线颜色；IMK 或应用可能重建并替换这些属性，因此候选窗第二行同时用红色和下划线显示异常拼音片段。仅错误时显示，修正后清除，不改变输入文本、选区、两行高度、Tab 补全和品牌区。长输入截取首个错误附近的片段，完整拼音保留在提示文本中。正常音节、未完成声母和双拼编码按各自方案判断；重复元音只在有足够输入证据时提示。

核心对齐 Windows 1.26.1，保留 Linux 的 FPC 适配与 macOS 的组合串语义。预测前缀准入由 `nc_prefix_completion_policy` 管理，候选快照和导航稳定性由 `nc_candidate_presentation` 管理；鼻音边界、已确认后缀、短复合词上下文证据、ü 别名和冷启动分页均有独立回归。共享轻量排序的两个局部缓存标志在入口显式初始化。

模型参数和九个 ONNX 图与 Windows 1.26.1 的部署资产相同，ORT 1.20.1 固定 CPU 执行、量化精度配置和线程/内存策略。生产补全结果预算为 50 ms，局部修正为 30 ms；主排序器的 30 ms 仅记录审计，保留已经同步算出的结果，与 Linux 宿主一致。没有引入云端、GPU/CoreML 执行器或重新训练。
...
基础数据库只读，用户数据库使用独立可写路径。Lexicon 1.26.1 的 24 个输入通过 schema 24 导入器重建简繁两库，完整保留 query paths、转移、补全竞争/审计和正反字符语言模型；模型与 schema 的 SHA-256 在 `data/runtime-assets.sha256`，词库生成输入校验值在 `data/lexicon-inputs.json`。
